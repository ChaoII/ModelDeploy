...
- [x] **服务化增强（嵌入式推理网关）**：内置 HTTP 推理端点、manifest 模型目录 + 单槽懒加载、内置通用模型构造、健康/就绪/鉴权/限流/CORS/metrics/TLS(可选)、访问日志、优雅停机——**本期 C++ 完整实现（REST）**，见 [Serving 使用指南](docs/serving.md)；**gRPC 二期**
