...
- **C#（`csharp/ModelDeployExample`）**：1 个，基于 capi 的 C# 绑定，编译运行正常但多数测试被注释、缺新 API 演示
...
### A. capi → capi 重写（22 个 `*_capi.cpp`）

统一模板，基于 `capi/md_capi.h` 的通用分发 API：
...
int main() {
    ...
    md_image_save(img, "capi_out.jpg");
...
- 每个 example 按 `MDModelKind` 对应，model_path 用 `|` 聚合串（OCR/LPR/insightface/audio 同 capi 规范）
