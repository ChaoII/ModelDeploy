...
MDStatusCode md_create_face_gender_model(MDModel* model, const char* model_path,
    ...
    const auto face_gender_model = new modeldeploy::vision::face::SeetaFaceGender(model_path, _option);
...
MDStatusCode md_face_gender_predict(const MDModel* model, MDImage* image, MDFaceGenderResult* c_result) {
    ...
    int gender_id;
    if (const bool res_status = face_gender_model->predict(image_data, &gender_id); !res_status) {
