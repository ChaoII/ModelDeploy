...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_create_detection_model(
    MDModel* model,
...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_set_detection_input_size(
    const MDModel* model,
    MDSize size);
...
/// \param image 原始图像
/// \return MDStatusCode::Success 成功，其他失败
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_detection_predict(
    const MDModel* model,
    MDImage* image,
...
MODELDEPLOY_CAPI_EXPORT void md_draw_detection_result(
    const MDImage* image,
    const MDDetectionResults* c_results,
    double threshold,
...
MODELDEPLOY_CAPI_EXPORT void md_free_detection_result(
    MDDetectionResults* c_results);

/// 释放检测模型
/// @param model 由create_detection_model创建的模型
MODELDEPLOY_CAPI_EXPORT void md_free_detection_model(
    MDModel* model);
