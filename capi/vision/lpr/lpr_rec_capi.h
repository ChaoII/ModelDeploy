...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_create_lpr_rec_model(
...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_lpr_rec_predict(
    const MDModel* model, MDImage* image,
    MDLPRResults* c_result);

/// 打印检测结果
MODELDEPLOY_CAPI_EXPORT void md_print_lpr_rec_result(
    const MDLPRResults* c_result);

/// 释放检测结果
/// @param c_result 由md_detection_predict生成的检测结果
MODELDEPLOY_CAPI_EXPORT void md_free_lpr_rec_result(
    MDLPRResults* c_result);

/// 释放检测模型
/// @param model 由create_detection_model创建的模型
MODELDEPLOY_CAPI_EXPORT void md_free_lpr_rec_model(MDModel* model);
