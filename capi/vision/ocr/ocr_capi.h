...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_create_ocr_model(MDModel* model, const MDOCRModelParameters* parameters);
...
/// \return 成功时返回MDStatusCode::Success，其他为失败
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_ocr_model_predict(const MDModel* model, MDImage* image, MDOCRResults* results);
...
/// 打印文本识别结果
MODELDEPLOY_CAPI_EXPORT void md_print_ocr_result(const MDOCRResults* results);
...
/// 释放文本识别结果
/// @param results 模型识别结果，通过md_ocr_model_predict方法手动开辟了内存空间，需要手动释放
MODELDEPLOY_CAPI_EXPORT void md_free_ocr_result(MDOCRResults* results);
