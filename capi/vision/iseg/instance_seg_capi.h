...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_create_instance_seg_model(
...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_set_instance_seg_input_size(
    const MDModel* model,
    MDSize size);
...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_iseg_predict_nv12(
    const MDModel* model,
...
/// 释放检测模型
/// @param model 由create_instance_seg_model创建的模型
MODELDEPLOY_CAPI_EXPORT void md_free_instance_seg_model(
    MDModel* model);
