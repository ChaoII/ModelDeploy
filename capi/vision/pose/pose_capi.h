...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_create_keypoint_model(
...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_set_keypoint_input_size(
    const MDModel* model, MDSize size);

MODELDEPLOY_CAPI_EXPORT MDStatusCode md_set_keypoint_num(
    const MDModel* model, int num);

/// 执行预测，检测结果保存在results中
/// \param model 由create_keypoint_model创建的模型
/// \param c_results 检测结果结构体，请在调用该方法前申请内存，该方法会在内部申请内存，并释放
/// \param image 原始图像
/// \return MDStatusCode::Success 成功，其他失败
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_keypoint_predict(
    const MDModel* model,
    MDImage* image,
    MDKeyPointResults* c_results);
/// NV12 直接输入预测（硬解码/摄像头直通，省去 BGR 转换）。
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_pose_predict_nv12(
    const MDModel* model,
...
/// 打印检测结果
MODELDEPLOY_CAPI_EXPORT void md_print_keypoint_result(
...
MODELDEPLOY_CAPI_EXPORT void md_free_keypoint_result(
    MDKeyPointResults* c_results);

/// 释放检测模型
/// @param model 由create_pose_model创建的模型
MODELDEPLOY_CAPI_EXPORT void md_free_keypoint_model(
    MDModel* model);
