...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_set_obb_input_size(
    const MDModel* model,
    MDSize size);
...
MODELDEPLOY_CAPI_EXPORT MDStatusCode md_obb_predict_nv12(
    const MDModel* model,
...
/// 释放检测模型
/// @param model 由create_obb_model创建的模型
MODELDEPLOY_CAPI_EXPORT void md_free_obb_model(
    MDModel* model);
