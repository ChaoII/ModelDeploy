...
- [ ] **Step 1: 写失败测试（设备帧 op fast-fail 不建 backend）**
...
- [ ] **Step 1: 迁移调用点（13 处 from_device_planes）**
...
- [ ] **Step 1: toCpu / cvt_color / codec 拒绝**
...
- [ ] **Step 2: CPU-only NV12→BGR 补测（修 #5）**
...
- [ ] **Step 2: 清理僵尸**：删除 `benchmark/benchmark_image_data.cpp`、`benchmark/benchmark_yolo_preproc.cpp`（登记册 #20），若 main 仍引用则同步移除。`keeper` 若新存储已用 owner 则删旧字段（#17）。
- [ ] **Step 3: 文档**：`docs/preprocess.md:7` 改描述；`docs/image_data_issue_register.md` 更新已消解项状态 + 基线。
- [ ] **Step 4: Build + 回归**

Run: `cmake --build build_tdc --target test_modeldeploy` 全绿；`.\test_modeldeploy.exe "~[model]"` ≥1094/113 全绿。
- [ ] **Step 5: Commit** `chore(image): align consumers to unified storage; remove zombie benchmark; update docs`
...
- [ ] **Step 2: 像素级等价抽查**：NV12→BGR、I420→BGR（yuv420p）、RGB24→BGR、`from_bgr24` 与 `md_image_from_bgr24` 逐位一致。
- [ ] **Step 3: 设备帧 fast-fail 抽查**：GPU/TPU 帧 `crop/resize/rotate/cvt_color/rotate_crop` + capi `show/save/encode/draw_*` → 均报错不崩不静默。
- [ ] **Step 4: Commit**（如有修正）
- [ ] **Step 5: 登记册/ spec 状态收尾**：勾 `docs/superpowers/plans/2026-08-18-image-data-unified-plane-storage.md` 全部 checkbox；更新 `docs/image_data_issue_register.md` 各消解项状态。
