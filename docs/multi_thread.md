...
`clone()` 的多线程方案**不保证单 GPU 吞吐提升**。对于 GPU 已饱和的场景，多线程甚至会略微降低吞吐。

实测（RTX 4060 Ti, YOLO11n 640×640, FP16）：

| 方案 | 400帧耗时 | 等效 FPS | 加速比 |
|------|----------|---------|--------|
| 单线程 | 4804ms | 83 | 1× |
| 4 路 clone | 5927ms | 67 | 0.81× |

多线程的价值在于：

1. **多路视频流同时处理**（20 路 RTSP，每路各自跑推理）
2. **CPU/GPU 延迟隐藏**——一路的预处理可与其他路的推理重叠
3. **多 GPU 扩展**——不同线程绑定不同 GPU 设备，吞吐线性增长

单个轻量模型在单 GPU 上，单线程已能喂饱 GPU。clone 的收益不在「单路跑更快」，而在「多路同时跑不互相阻塞」。
