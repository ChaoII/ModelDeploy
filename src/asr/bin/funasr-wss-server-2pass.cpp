...
void GetValue(TCLAP::ValueArg<std::string> &value_arg, const string& key,
...
#include <windows.h>
    ...
    try {
        ...
        TCLAP::ValueArg<float> lattice_beam("", LAT_BEAM, "the lattice generation beam for beam searching ", false, 3.0,
                                            "float");
        ...
        try {
            ...
            if (!s_offline_asr_path.empty()) {
                ...
                if (ret != 0) {
                    LOG(INFO)
                            << "Failed to download model from modelscope. If you set local asr model path, you can ignore the errors.";
                }
            ...
            if (!s_lm_path.empty() && s_lm_path != "NONE" && s_lm_path != "none") {
                ...
                if (ret != 0) {
                    LOG(INFO)
                            << "Failed to download model from modelscope. If you set local lm model path, you can ignore the errors.";
                }
            ...
            if (!s_punc_path.empty()) {
                ...
                if (ret != 0) {
                    LOG(INFO)
                            << "Failed to download model from modelscope. If you set local punc model path, you can ignore the errors.";
                }
            ...
            if (!s_itn_path.empty()) {
                ...
                if (ret != 0) {
                    LOG(INFO)
                            << "Failed to download model from modelscope. If you set local itn model path, you can ignore the errors.";
                }
        ...
        ExtractHotWords(hotword_path, hws_map_);
        ...
        WebSocketServer websocket_srv(io_decoder, is_ssl, server, wss_server, s_certfile,s_keyfile);  // websocket server for asr engine
