...
/// \return 0 成功，其他失败
EXPORT_DECL MDStatusCode md_set_detection_input_size(MDModel *model, MDSize size);
/// 执行预测，检测结果保存在results中
/// \param model 由create_detection_model创建的模型
/// \param results 检测结果结构体，请在调用该方法前申请内存，该方法会在内部申请内存，并释放
/// \param image 原始图像
/// \return 0 成功，其他失败
EXPORT_DECL MDStatusCode md_detection_predict(MDModel *model, MDImage *image, MDDetectionResults *results);

/// 打印检测结果
EXPORT_DECL void md_print_detection_result(MDDetectionResults *result);
...
EXPORT_DECL void md_draw_detection_result(MDImage *image, MDDetectionResults *result, const char *font_path, int font_size,
