    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            for i in range(im_infos.shape[0]):
                im_infos[i] = pickle.loads(im_infos[i])
            results = self.postprocessor_.run([infer_outputs], im_infos)
            list0 = []
            for _results in results:
                list1=[]
                for result in _results:
                    list1.append(result.to_dict())
                list0.append(json.dumps(list1))
            print("results_dict: ", list0)
            out_tensor = pb_utils.Tensor(self.output_names[0], np.array(list0, dtype=np.object_))
