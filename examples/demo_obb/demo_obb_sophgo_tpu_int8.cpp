// ModelDeploy demo: 旋转目标检测（sophgo_tpu_int8）。
...
int main() {
    ...
    // ---- 2. 加载模型（旋转目标检测）----
    auto m = std::make_unique<modeldeploy::vision::detection::UltralyticsObb>("../../test_data/test_models/sophgo/yolo26n/yolo26n-obb-int8.bmodel", opt);
    ...
    (void)vis.imwrite("result_obb_sophgo_tpu_int8.jpg");
