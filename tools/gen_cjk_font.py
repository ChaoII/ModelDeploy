...
def main(font_path: str, out_header: str) -> None:
    ...
    for i, src in enumerate(order):
        new_glyphs.append(glyphs[src * SIZE * 2:(src + 1) * SIZE * 2])
    flat = [b for g in new_glyphs for b in g]
    lines = []
    lines.append("#pragma once")
    lines.append("#include <cstdint>")
    lines.append(f"// Generated by tools/gen_cjk_font.py; {len(index)} glyphs, {SIZE}x{SIZE} px, stride=2 bytes/row")
    ...
    lines.append(f"inline const uint8_t kCjkFontGlyphWidth  = {SIZE};")
    lines.append(f"inline const uint8_t kCjkFontGlyphRows   = {SIZE};")
    lines.append(f"inline const uint8_t kCjkFontGlyphStride = 2;")
