...
#include "result.h"
#include <iostream>
#include <csrc/vision/detection/yolov8.h>
...
namespace modeldeploy::vision {
    ...
    std::string FaceDetectionResult::Str() {
        ...
        if (!(landmarks.size() == boxes.size() * landmarks_per_face)) {
    ...
    void MattingResult::Reserve(int size) {
        ...
        if (contain_foreground) {
            if (!(shape.size() == 3)) {
                std::cerr << "Please initial shape (h,w,c) before call Reserve." << std::endl;
            }
    ...
    void MattingResult::Resize(int size) {
        ...
        if (contain_foreground) {
            if (!(shape.size() == 3)) {
                std::cerr << "Please initial shape (h,w,c) before call Resize." << std::endl;
            };
            int c = static_cast<int>(shape[2]);
