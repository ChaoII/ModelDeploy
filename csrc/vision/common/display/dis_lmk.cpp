...
namespace modeldeploy::vision {
    void dis_lmk(const std::vector<DetectionLandmarkResult>& result) {
        ...
        for (size_t i = 0; i < result.size(); ++i) {
            ...
            for (size_t j = 0; j < result[i].landmarks.size(); ++j) {
                row_str_landmarks += "[" +
                    std::to_string(static_cast<int>(result[i].landmarks[j].x)) + "," +
                    std::to_string(static_cast<int>(result[i].landmarks[j].y));
                row_str_landmarks += j < result[i].landmarks.size() - 1 ? "], " : "]";
            }
