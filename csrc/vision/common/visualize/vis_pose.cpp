...
namespace modeldeploy::vision
{
    ...
    cv::Mat vis_pose(cv::Mat& cv_image, const PoseResult& result,
        ...
        for (int i = 0; i < result.boxes.size(); ++i) {
            ...
            std::vector current_keypoints(
                result.keypoints.begin() + i * result.keypoints_per_instance,
                result.keypoints.begin() + (i + 1) * result.keypoints_per_instance
            );
            draw_keypoints(cv_image, current_keypoints, landmark_radius);
        }
