...
namespace modeldeploy::vision {
    void draw_rectangle_and_text(cv::Mat& image, const std::array<float, 4> box, const std::string & plate_color,
                             const std::string& plate_str,
                             const float score,
...
cv::Mat vis_lpr(cv::Mat& cv_image, const LprResult& result,
    ...
    for (int i = 0; i < result.boxes.size(); ++i) {
        draw_rectangle_and_text(overlay, result.boxes[i], result.car_plate_colors[i], result.car_plate_strs[i], result.scores[i], cv_color, font, font_size, -1, false);
    }
    cv::addWeighted(overlay, alpha, cv_image, 1 - alpha, 0, cv_image);
    // 绘制对象矩形矩形边框、文字背景边框、文字、关键点
    for (int i = 0; i < result.boxes.size(); ++i) {
        draw_rectangle_and_text(cv_image, result.boxes[i], result.car_plate_colors[i], result.car_plate_strs[i],result.scores[i], cv_color, font, font_size, 1, true);
        for (size_t j = 0; j < 4; ++j) {
            cv::Point landmark;
            auto landmark_color = get_random_color();
            landmark.x = static_cast<int>(
                result.landmarks[i * 4 + j][0]);
            landmark.y = static_cast<int>(
                result.landmarks[i * 4 + j][1]);
