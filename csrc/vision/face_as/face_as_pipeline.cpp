...
namespace modeldeploy::vision::face {
    ...
    static float re_blur(const unsigned char *data, int width, int height) {
        float blur_val = 0.0;
        float kernel[9] = {1.0 / 9.0, 1.0 / 9.0, 1.0 / 9.0, 1.0 / 9.0, 1.0 / 9.0, 1.0 / 9.0, 1.0 / 9.0, 1.0 / 9.0,
                           1.0 / 9.0};
    ...
    bool SeetaFaceAsPipeline::predict(cv::Mat &im,
        ...
        DetectionLandmarkResult face_det_result;
        bool has_box = !face_as_second_result.empty();
        if (!face_det_->predict(im_bak1, &face_det_result)) {
            return false;
        }
        face_det_result.display();
        results->resize(face_det_result.boxes.size());
        ...
        for (auto &align_image: align_im_list) {
            ...
            auto clarity = clarity_estimate(align_image);
            face_as_first_->predict(align_image, &passive_result);
            float result = has_box ? 0.0f : passive_result;
