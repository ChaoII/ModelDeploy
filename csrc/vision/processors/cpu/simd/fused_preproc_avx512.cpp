...
MD_TARGET_AVX512 void fused_color_matrix_avx512(const uint8_t* src, int src_w, int src_h,
                                                float* dst, int dst_w, int dst_h,
                                                float origin_x, float origin_y,
                                                float scale_x, float scale_y,
    ...
    for (int y = 0; y < dst_h; ++y) {
        ...
        for (; x + 16 <= dst_w; x += 16) {
            ...
            const __m512 rv = _mm512_loadu_ps(r);
            const __m512 gv = _mm512_loadu_ps(g);
            const __m512 bv = _mm512_loadu_ps(b);
            _mm512_storeu_ps(dst + 0 * plane + base + x,
                _mm512_fmadd_ps(_mm512_set1_ps(mat[0][0]), rv,
                    _mm512_fmadd_ps(_mm512_set1_ps(mat[0][1]), gv,
                        _mm512_fmadd_ps(_mm512_set1_ps(mat[0][2]), bv, bias0))));
            _mm512_storeu_ps(dst + 1 * plane + base + x,
                _mm512_fmadd_ps(_mm512_set1_ps(mat[1][0]), rv,
                    _mm512_fmadd_ps(_mm512_set1_ps(mat[1][1]), gv,
                        _mm512_fmadd_ps(_mm512_set1_ps(mat[1][2]), bv, bias1))));
            _mm512_storeu_ps(dst + 2 * plane + base + x,
                _mm512_fmadd_ps(_mm512_set1_ps(mat[2][0]), rv,
                    _mm512_fmadd_ps(_mm512_set1_ps(mat[2][1]), gv,
                        _mm512_fmadd_ps(_mm512_set1_ps(mat[2][2]), bv, bias2))));
        }
        for (; x < dst_w; ++x) {
            const float src_xf = static_cast<float>(x) * inv_scale_x - origin_shift_x;
            float rv, gv, bv;
