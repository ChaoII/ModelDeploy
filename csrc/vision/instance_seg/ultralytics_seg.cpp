...
namespace modeldeploy::vision::detection {
    ...
    bool UltralyticsSeg::predict(const cv::Mat& image, DetectionResult* result) {
    ...
    bool UltralyticsSeg::batch_predict(const std::vector<cv::Mat>& images, std::vector<DetectionResult>* results) {
        ...
        std::cout<<"reused_output_tensors_[1].shape[0]= " <<reused_output_tensors_[1].shape()[0]<<std::endl;
        std::cout<<"reused_output_tensors_[1].shape[1]= " <<reused_output_tensors_[1].shape()[1]<<std::endl;
        std::cout<<"reused_output_tensors_[1].shape[2]= " <<reused_output_tensors_[1].shape()[2]<<std::endl;
        std::cout<<"reused_output_tensors_[1].shape[3]= " <<reused_output_tensors_[1].shape()[3]<<std::endl;
