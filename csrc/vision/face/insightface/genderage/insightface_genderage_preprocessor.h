//
// insightface buffalo_l genderage 前处理。
// 与 landmark 相同：以 bbox 中心 transform（scale=96/(max(w,h)*1.5)）裁剪到 96x96。
...
namespace modeldeploy::vision::face {
    ...
    class MODELDEPLOY_CXX_EXPORT InsightFaceGenderAgePreprocessor {
    public:
        InsightFaceGenderAgePreprocessor();

        // 单人脸：输出 [1,3,96,96] FP32（0-255）；M 为前向仿射矩阵
