...
namespace modeldeploy::vision::utils {
    ...
    std::vector<cv::Mat> AlignFaceWithFivePoints(
        ...
        if (std_landmarks.size() != 5) { std::cerr << "The landmarks.size() must be 5."; }
        if (image.empty()) { std::cerr << "The input_image can't be empty."; }
        std::vector<cv::Mat> output_images;
        output_images.reserve(result.scores.size());
        if (result.boxes.empty()) {
            std::cerr << "The result is empty." << std::endl;
        ...
        for (int i = 0; i < result.landmarks.size(); i += 5) {
            ...
            if (cropped_image_aligned.empty()) {
                std::cerr << "croppedImageAligned is empty." << std::endl;
            }
