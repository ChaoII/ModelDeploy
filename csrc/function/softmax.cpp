...
namespace modeldeploy::function {
    ...
    template <typename T>
    struct SoftmaxEigen {
        void operator()(const MDTensor& x, MDTensor* out, const int axis_dim) {
    ...
    template <typename T>
    void SoftmaxKernel(const MDTensor& x, MDTensor* out, const int axis) {
        const int rank = static_cast<int>(x.shape.size());
        const int calc_axis = canonical_axis(axis, rank);
        const int axis_dim = static_cast<int>(x.shape[calc_axis]);
