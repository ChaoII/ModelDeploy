...
inline uint32_t getElementSize(nvinfer1::DataType t) noexcept {
    ...
    case nvinfer1::DataType::kFP4: MD_LOG_FATAL << "Element size is not implemented for sub-byte data-types" <<
            std::endl;
    }
    return 0;
}
    ...
    private:
        void* getBuffer(bool const isHost, std::string const& tensorName) const {
            ...
            if (record == mNames.end())
                return nullptr;
            return (isHost
                        ? mManagedBuffers[record->second]->hostBuffer.data()
        ...
        void memcpyBuffers(bool const copyInput, bool const deviceToHost, bool const async,
