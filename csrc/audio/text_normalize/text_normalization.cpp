...
namespace modeldeploy::audio {
    ...
    TextNormalizer::TextNormalizer(const std::filesystem::path& char_map_folder)
        ...
        MD_LOG_INFO << "TextNormalizer is constructed!" << std::endl;
    }

    // 分割函数
    std::vector<std::wstring> TextNormalizer::split(const std::wstring& text, const std::wstring& lang) const {
    ...
    std::wstring TextNormalizer::normalize_sentence(const std::wstring& sentence) {
        ...
        modified_sentence = full_width_to_half_width(modified_sentence); // constants 全角转半角
        ...
        // 固话
        while (std::regex_search(modified_sentence, match, re_telephone) && is_valid_phone_number(
            modified_sentence, match))
