...
namespace modeldeploy::audio {
    ...
    // Note that there is no minus symbol '-' here. The hyphen could be
    ...
    std::wstring replace_default_num(const std::wsmatch& match) {
        const std::wstring number = match.str(0);
        std::wstring result;
    ...
    std::wstring replace_asmd(const std::wsmatch& match) {
        const std::wstring result = match.str(1) + asmd_map[match.str(8)[0]] + match.str(9);
        return match.prefix().str() + result + match.suffix().str();
    }
    ...
    std::wstring replace_positive_quantifier(const std::wsmatch& match) {
        ...
        const std::wstring quantifiers = match.str(3);
        number = num2str(number);
    ...
    std::wstring replace_number(const std::wsmatch& match) {
        std::wstring sign = match.str(1);
        std::wstring number = match.str(2);
        const std::wstring pure_decimal = match.str(5);
    ...
    std::vector<std::wstring> _get_value(const std::wstring& value_string, bool use_zero) {
        ...
        const size_t first_non_zero = stripped.find_first_not_of(L'0');
        if (first_non_zero != std::wstring::npos) {
            stripped.erase(0, first_non_zero);
        }
        else {
            stripped = L"0"; // 全是0的情况，保留一个零
        }
        if (stripped.empty()) {
            return {};
        }
        ...
        const std::wstring first_part = value_string.substr(0, value_string.size() - largest_unit);
        const std::wstring second_part = value_string.substr(value_string.size() - largest_unit);
    ...
    std::wstring verbalize_digit(const std::wstring& value_string, const bool alt_one) {
        std::wstring result;
    ...
    std::wstring num2str(const std::wstring& value_string) {
        const size_t point_pos = value_string.find(L'.');
