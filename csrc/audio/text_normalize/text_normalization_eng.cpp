...
namespace modeldeploy::audio {
    ...
    static std::string int_to_words_helper(const int num) {
        if (num == 0)
            return "";
        if (num < 20)
    ...
    static std::string decimal_to_words(const double decimalPart) {
        if (decimalPart == 0)
            return "";
        std::ostringstream oss;
        oss << std::fixed << decimalPart;
        const std::string decimalStr = oss.str().substr(2);
        std::string result = "point";
        for (const char c : decimalStr) {
    ...
    static std::string number_to_words(const double num) {
        const double x = std::floor(num);
        const int intPart = static_cast<int>(x);
        const double decimalPart = num - intPart;
        const std::string words = int_to_words(intPart);
        const std::string decimalWords = decimal_to_words(decimalPart);
    ...
    static std::string convert_ordinal(const int num) {
        ...
        const int tens = num / 10 * 10; // Get the tens place
    ...
    std::string normalize_numbers(const std::string& text) {
        ...
        while (std::regex_search(result, match, ordinal_re)) {
            const int num = std::stoi(match.str(0));
    ...
    static std::string _expand_time_english(const std::smatch& match) {
        ...
        const int minute = std::stoi(match.str(6));
        if (minute > 0) {
            if (minute < 10) {
                result += " oh";
            }
        ...
        const std::string am_pm = match.str(7);
        if (am_pm.empty()) {
            result += past_noon ? " p m " : " a m ";
        }
        else {
            for (const char c : am_pm) {
                if (c != '.') {
                    result += " ";
                    result += c;
                }
