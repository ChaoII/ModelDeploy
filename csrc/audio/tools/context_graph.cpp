// context_graph.cpp —— 移植自 sherpa-onnx/csrc/context-graph.cc（Copyright 2023 Xiaomi Corp., Apache-2.0）
...
namespace {
void check_size(size_t got, size_t expected, const char* what) {
    if (got != expected)
        throw std::invalid_argument(std::string("ContextGraph: ") + what + " size mismatch");
}
} // namespace
...
ContextGraph::ForwardOneStep(const ContextState* state, int32_t token,
    ...
    if (!node) throw std::runtime_error("ContextGraph::ForwardOneStep: bad state");
...
void ContextGraph::FillFailOutput() const {
    ...
    while (!node_queue.empty()) {
        ...
        node_queue.pop();
        for (const auto& kv : current_node->next) {
